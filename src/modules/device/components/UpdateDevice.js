...
const UpdateDevice = () => {
  ...
  const simulateDeviceFeedback = async () => {
    try {
      ...
      const feedback = MatterTunnel.extractTXDataWithoutSign(
        privateKey,
        response.data
      );
  ...
  const deviceFunctions = (device?.functions || [])
    .map((func) => {
  ...
  const handleExecute = async (funcName, paramTypes, paramValues) => {
    ...
    const TxBytes = MatterTunnel.makeTX(
      funcName,
      privateKey,
      publicKey,
      paramValues
      ...
      <div className={styles.deviceDetail}>
        ...
        {feedback && (
          <div className={styles.feedback}>
            ...
            <span className={styles.feedbackContent}>{feedback}</span>
          </div>
        )}
      </div>
      ...
      <div className={styles.functionList}>
        {deviceFunctions.map((func, index) => (
          <div key={index} className={styles.functionItem}>
            <label className={styles.functionLabel}>{func.name}</label>
            <div className={styles.functionInputs}>
              {func.parameters.map((paramType, pIndex) => (
                <input
                  ...
                  onChange={(e) =>
                    handleInputChange(`${func.name}_${pIndex}`, e.target.value)
                  }
              ...
              <button
                className={`${styles.executeButton} ${
                  loadingStates[func.name] ? styles.loading : ""
                }`}
                onClick={() =>
                  handleExecute(
                    func.name,
                    func.parameters,
                    func.parameters.map(
                      (_, pIndex) => inputValues[`${func.name}_${pIndex}`]
                    )
                  )
                }
                disabled={loadingStates[func.name]}
